...
def titulo_tema(tema):
    if tema == "MF0219_UF1_UD2_T1":
        st.info(f"##### 1. Conceptos básicos. Concepto y arquitectura del SO. ")
    elif tema == "MF0219_UF1_UD3_T2":
        st.info(f"##### 2. Funciones del SO. Gestión de recursos. ")
...
def tabs_tema(tema):
    ...
    with desarrollo:
        ...
        if tema == "MF0219_UF1_UD2_T1":
            ...
            #contenido = leer_markdown("pages/pagina.md")
            #st.markdown(contenido)
            leccion_viewer(clave_unica=tema,  pagina_fija="MF0219_UF1_UD2_T2")
        else:
            st.write("error")
